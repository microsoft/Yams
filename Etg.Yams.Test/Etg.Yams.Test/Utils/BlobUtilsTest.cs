...
namespace Etg.Yams.Test.Utils
{
    ...
    public class BlobUtilsTest
    {
        ...
        public async Task TestDeleteBlobDirectory()
        {
            ...
            Assert.IsFalse(root.ListBlobs(true).Any());
        }
        ...
        public async Task TestUploadFile()
        {
            ...
            Assert.AreEqual(testFileContent, await blob.DownloadTextAsync());
        }
        ...
        private static void VerifyThatAllFilesAreThere(ISet<string> relativePathSet, string separator)
        {
            ...
            Assert.IsTrue(relativePathSet.Contains($"d1{separator}b1"));
            Assert.IsTrue(relativePathSet.Contains($"d1{separator}b2"));
            Assert.IsTrue(relativePathSet.Contains($"d2{separator}b3"));
            Assert.IsTrue(relativePathSet.Contains($"d2{separator}b4"));
            Assert.IsTrue(relativePathSet.Contains($"d2{separator}d3{separator}b5"));
            Assert.IsTrue(relativePathSet.Contains($"d2{separator}d3{separator}b6"));
            Assert.IsTrue(relativePathSet.Contains($"d4{separator}d5{separator}b7"));
        }

        private static async Task<CloudBlobDirectory> CreateBlobsTree()
        {
            // root
            // |__b1
            // |__b2
