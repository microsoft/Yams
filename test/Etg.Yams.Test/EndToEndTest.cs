...
namespace Etg.Yams.Test
{
    public class EndToEndTest : IDisposable
    {
        ...
        public void AssertThatApplicationIsRunning(AppIdentity appIdentity)
        {
            ...
            Assert.True(applicationPool.HasApplication(appIdentity), string.Format("App {0} should be running!", appIdentity));
        ...
        {
            IApplicationPool applicationPool = _diContainer.Resolve<IApplicationPool>();
            Assert.False(applicationPool.HasApplication(appIdentity), string.Format("App {0} should not be running!", appIdentity));
